...
if (analysis_mode == "publication") {
    DATASET <- cclong
    settings <- list(n.chains= 4,  n.adapt= 500, burnin = 5000, n.iter= 25000, thin=20) # around 8 hours - multicore
}
...
modelfit <- list()
getid <- which(model_specifications$random_error == TRUE & 
              model_specifications$lag_effect == FALSE &
              model_specifications$linear_quadratic == FALSE & 
              model_specifications$dv == "pwi_mean_probit")
modelfit[[model_specifications[getid, "name"]]] <- fityhat_jags_model(getid)

getid <- which(model_specifications$random_error == TRUE & 
              model_specifications$lag_effect == FALSE &
              model_specifications$linear_quadratic == TRUE & 
              model_specifications$dv == "pwi_mean_probit")
modelfit[[model_specifications[getid, "name"]]] <- fityhat_jags_model(getid)
...
modelfit$rerr_lagFA_int_pwi_mean_probit$data$
...
merge_yhats <- function(model) {
    ...
    temp <- merge(yhv$combined, DATASET, by.x='observationid', by.y="wave_id")
    DATASET$wave_id
...
pdf(file="output/individual-fits.pdf", width=10, height=10)
grid.arrange(pcomb$rerr_lagFA_int_pwi_mean_probit, pcomb$rerr_lagFA_linquad_pwi_mean_probit, ncol=2)
dev.off()
