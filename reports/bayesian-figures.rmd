...
yhv$combined$observationid <-  modelfit$data$observationid

yhd <- merge(yhv$combined, DATASET, by.x='observationid', by.y="wave_id")


plot_subset <- function(subset, filename="output/individual-fits.pdf") {
    yhds <- yhd[ yhd$user_id %in% subset, ]
    # order by ocitotal_z
#      ocisub <- yhds[, c('user_id', )]
#      ocisub <- ocisub[order(ocisub$ocitotal_z), ]
     # ocisub$order <- seq(nrow(ocisub))
     # yhds <- merge(yhds, ocisub[,c('subject_id', 'order')])
...
plot_subset(subset = w10)
dim(yhd)
head(yhd)
dim(modelfit$samples)



plot(aggregate(yhat_mean ~ round(age_est), yhd, mean))
```
